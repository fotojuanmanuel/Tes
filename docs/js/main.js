...
function setupSearch() {
    ...
    setupDictation(searchInput, searchFeedback);
...
function setupDictation(searchInput, searchFeedback) {
    ...
    if (!SpeechRecognition) {
        ...
        dictationBtn.setAttribute('aria-disabled', 'true');
        dictationBtn.title = 'Dictado no disponible en este navegador';
        return;
    }
    ...
    dictationBtn.addEventListener('click', function () {
        ...
        try {
            ...
            setFeedback(searchFeedback, 'No se pudo iniciar el dictado. Intenta nuevamente.', 'error');
        }
    });

    recognition.addEventListener('start', function () {
        isListening = true;
        dictationBtn.classList.add('is-listening');
        dictationBtn.setAttribute('aria-pressed', 'true');
        setFeedback(searchFeedback, 'Escuchando dictado...', 'success');
    });

    recognition.addEventListener('result', function (event) {
        const result = event.results[event.resultIndex];
        const transcript = result && result[0] ? result[0].transcript : '';
        searchInput.value = removeFinalDictationMark(transcript);
        searchInput.focus();
        searchInput.dispatchEvent(new Event('input', { bubbles: true }));
        setFeedback(searchFeedback, 'Dictado agregado.', 'success');
    });

    recognition.addEventListener('error', function () {
        setFeedback(searchFeedback, 'No se pudo completar el dictado. Intenta nuevamente.', 'error');
    });

    recognition.addEventListener('end', function () {
        isListening = false;
        dictationBtn.classList.remove('is-listening');
        dictationBtn.setAttribute('aria-pressed', 'false');
    });
}

function removeFinalDictationMark(value) {
